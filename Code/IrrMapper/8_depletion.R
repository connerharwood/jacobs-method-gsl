...
fields_panel = st_read("Data/IrrMapper/Utah/fields_irrmapper_panel_final.gpkg") |> 
  st_drop_geometry()

load("Data/IrrMapper/Utah/masterdata.rda")
...
median(depletion_annual$depletion_ft, na.rm = TRUE) # 1.965253 vs 1.504554 with stacked IrrMapper
median(depletion_annual$depletion_af, na.rm = TRUE) # 13.20108

# ==== SAVE ====================================================================

save(depletion_monthly, file = "Data/IrrMapper/Utah/depletion_monthly.rda")
save(depletion_annual, file = "Data/IrrMapper/Utah/depletion_annual.rda")

check = depletion_monthly |> 
  filter(!is.na(depletion_ft)) |> 
  group_by(year) |> 
  count()

fields = st_read("Data/Clean/Fields/Utah/fields_panel.gpkg")
field_id = unique(fields$id)
n_distinct(depletion_monthly$id)
n_distinct(field_id)
