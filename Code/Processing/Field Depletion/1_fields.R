...
for (year in 2017:2024) {
  ...
  shapefile_raw = list.files(
    path = file.path("Data/Raw/Fields/Raw WRLU", year),
  ...
  shapefile_repaired = file.path("Data/Raw/Fields/Repaired WRLU", year, paste0("wrlu_", year, ".shp"))
...
for (yr in 2017:2024) {
  # Shapefile path
  file_path = glue("Data/Raw/Fields/Repaired WRLU/{yr}/wrlu_{yr}.shp")
  ...
  if (yr == 2024) {
    ...
    wrlu_2024 = st_read(file_path) |> 
      ...
      # Create unique ID for each field
...
st_write(fields_panel_temp, "Data/Clean/fields_panel_temp.gpkg", delete_dsn = TRUE)
