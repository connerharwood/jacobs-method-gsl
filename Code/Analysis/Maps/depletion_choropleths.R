...
counties = st_read("Data/Raw/Counties/Counties.shp") |> 
  ...
  #st_intersection(gsl_basin) |> 
  #filter(!county %in% c("Carbon", "Duchesne", "Sanpete")) |> 
  #group_by(county) |> 
...
# Load cities and towns within GSL Basin counties
cities = st_read("Data/GSLCO/Depletion Estimates/cities_towns.gpkg") |> 
  # Validate geometry
  st_make_valid() |> 
  # Transform to NAD 83 for spatial operations and plotting
  st_transform(crs = 26912) |> 
  # Filter to cities and towns within GSL Basin counties
  st_filter(counties, .predicate = st_within) |> 
  # Select name and type
  select(city = NAME, type = TYPE) |> 
  # Grrab X and Y coordinates for labeling on map
  mutate(
...
county_labels_huc12 = county_depletion |> 
  st_point_on_surface() |> 
  ...
  cbind(county = county_depletion$county) |> 
...
cache_field_depth_choro = ggplot() +
  ...
  # geom_shadowtext(
  #   data = cities |> st_filter(cache) |> filter(type == "City"),
...
#     legend.text = element_text(size = 12), # Adjust legend tick labels
#     plot.title = element_text(size = 20, hjust = 0.5), # Adjust plot title
#     legend.position = c(0.84, 0.914), # Adjust legend position
#     legend.justification = c(0, 1) # Adjust legend position
#   ) +
#   # Adjust color scale bar
#   guides(
...
# Manually designate where to label county names on map
county_labels = county_depletion |> 
  st_point_on_surface() |> 
  # Grab X and Y coordinates
  st_coordinates() |>
  # Convert to df
...
# Manually designate where to label county names on map
county_labels_company = county_depletion |> 
  st_point_on_surface() |> 
  # Grab X and Y coordinates
  st_coordinates() |>
  # Convert to df
